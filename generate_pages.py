...
def render_candidate(hire: dict) -> str:
    ...
    if hire.get("has_intro_video") == "Yes" and video_url:
        ...
        video = ""
    agreed_salary = hire.get("agreed_monthly_salary")
    salary = (
        f'<span class="salary-chip">Hired at ${int(agreed_salary):,}/mo</span>'
        if agreed_salary
        else ""
    )
...
<main>
  ...
  <section class="section data-room" id="sources">
    ...
    <div class="caveat-box">
      ...
      <p>Sixteen candidate-specific agreed salaries are verified from CORE placement-fee records and shown on the relevant cards. Unverified compensation stays hidden. Candidate profile asking rates are not presented as final pay.</p>
...
.salary-chip{display:inline-block;margin-top:9px;padding:4px 8px;border:1px solid var(--line);border-radius:999px;background:var(--white);color:var(--glacier);font:10px var(--mono);white-space:nowrap}
